...
var (
	rootCmd = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			logger.Debug("rootCmd.Run 被调用",
				zap.Strings("args", args),
				zap.Bool("sendWeekly", sendWeeklyFlag),
				zap.Bool("sendMonthly", sendMonthlyFlag),
				zap.Bool("sendYearly", sendYearlyFlag),
				zap.Bool("sendDaily", sendDailyFlag))
			...
			if sendWeeklyFlag || sendMonthlyFlag || sendYearlyFlag || sendDailyFlag {
				logger.Debug("进入 RunSendReports 分支")
				app.RunSendReports(ctx, opts)
			} else {
				logger.Debug("进入 RunTests 分支")
				app.RunTests(ctx, args, opts)
			}
			logger.Debug("rootCmd.Run 执行完毕")
		},
	}
